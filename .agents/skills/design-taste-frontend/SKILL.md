...
* **Long lists need a different UI component, not a longer list.** Default `<ul>` with bullets / `divide-y` rows is the lazy choice. If you have > 5 items, reach for one of these instead:
  ...
  A spec sheet with 10 rows + a hairline under every row is the WORST default. Either group rows into 2-3 chunks with sparse dividers, or move to a warm-beige or other styled layout.
...
* **Max 3 lines** of quote text. If the original quote is longer, edit/truncate it to fit or use a disclosure element for the full quote.
* Attributions should be clear and clean. Avoid generic star ratings or heavy quote-mark graphics.
